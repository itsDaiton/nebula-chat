import { createLogger, logEvent } from '@nebula-chat/otel';
...
const logger = createLogger({ serviceName: 'nebula-chat-server' });
...
export const loadServerEnv = (): void => {
  try {
    ...
    logEvent(
      ...
      'env.file.missing',
      {},
      'No .env file found for nebula-chat-server, falling back to process.env',
    );
  }
};
