...
const LIST_URL = 'http://localhost:3000/api/conversations';
...
const servePage = (
  ...
  server.use(http.get(LIST_URL, () => HttpResponse.json({ conversations, nextCursor, hasMore })));
};
...
describe('fetchConversations', () => {
  ...
  it('still clears the loading flag after a failure', async () => {
    server.use(http.get(LIST_URL, () => HttpResponse.json({}, { status: 500 })));
...
describe('loadMore', () => {
  ...
  it('records an error and clears the loading-more flag on failure', async () => {
    ...
    server.use(
      http.get(LIST_URL, () => HttpResponse.json({ message: 'Gateway timeout' }, { status: 504 })),
    );
