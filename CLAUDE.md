...
This is a TypeScript monorepo. Workspace packages live under `apps/` (runnable services) and `libs/` (shared internal libraries).
...
Standalone Drizzle ORM library consumed by `apps/nebula-chat-server` via `workspace:*`.

- **Schema**: `src/schema.ts` — `users`, `conversations`, `messages` tables with FK constraints and indexes.
- **Client**: `createDbClient(config)` returns `{ db, pool }` — `db` is the Drizzle query client, `pool` is the raw `pg.Pool` for graceful shutdown.
- **Types**: exports `DbClient` and `DbTransaction` for use in server repositories.
- **Migrations**: SQL files in `migrations/` generated by drizzle-kit. Applied via `db:migrate` in the Render build command.
- **Build**: dual ESM + CJS output via `tsup` so the CommonJS server can import it.

### Backend (`/apps/nebula-chat-server`)

Fastify 5 REST API with:

- **PostgreSQL + Drizzle**: Three tables — `users`, `conversations`, `messages` — defined in `@nebula-chat/db`. DB client exposed via `src/db.ts` singleton and `src/plugins/db.plugin.ts` Fastify plugin (`app.db`). Repositories in each module use Drizzle's SQL builder.
