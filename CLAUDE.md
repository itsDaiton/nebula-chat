...
- **Open a PR when the work is complete**, consistent with `AGENTS.md`. Use `gh pr create` with the primary Conventional Commit header as the title. When the work came from a ticket, carry its Jira-style ID — `type(scope): NEB-<n> "Ticket title"` (issue `#NN` is ticket `NEB-<n>`); for ad-hoc work with no ticket, use plain `type(scope): summary`. Because the repo squash-merges with `PR_TITLE`, **the PR title is the one line Release Please parses** — it must be `feat` or `fix` (`!` for a breaking/major change). The body must include: one-line summary, a `Closes #NN` line for the ticket (auto-close keys off the bare `#NN`, not `NEB-<n>` — see `AGENTS.md`'s Referencing tickets), test plan checklist, and a link to the ADR (`docs/adr/NNNN-*.md`) backing the change, if one exists.
